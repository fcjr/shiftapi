...
Use `WithGlobalError` with `New()` to register an error type on all routes:
...
This is useful for cross-cutting error types like authentication or rate-limiting errors. API-level and route-level error types are merged — every route gets both.
