...
- **Type-safe errors** — declare custom error types per route with `WithError[T](status)` or globally with `WithGlobalError[T](status)` — schemas appear in the OpenAPI spec and errors are matched at runtime via `errors.As`
