...
`Handle` is a method on `*API` and on `*Group`. Call it on whichever one the route belongs to:
...
api.Handle("GET /items", func(r *http.Request, _ struct{}) (*CachedResponse, error) {
    etag := `"v1"`
...
```go
api.Handle("DELETE /items/{id}", func(r *http.Request, in DeleteInput) (struct{}, error) {
    deleteItem(in.ID)
    return struct{}{}, nil
...
api.Handle("DELETE /items/{id}", func(r *http.Request, in DeleteInput) (DeleteResponse, error) {
    deleteItem(in.ID)
...
Once an API outgrows one file, register each set of routes from its own
function. Take a `*shiftapi.Group` and let the caller decide the prefix:
...
// billing.go
func BillingRoutes(g *shiftapi.Group) {
    g.Handle("GET /invoices", listInvoices)
}
...
Each function inherits the group's prefix, error types, middleware, and static
response headers, so `UserRoutes` above registers `GET /api/v1/users`. To mount
a set of routes at the root, pass `api.Group("")`.

Take `*shiftapi.Group` in these functions rather than `*shiftapi.API`. Go does
not allow generic methods on interfaces, so no single interface covers both
types, and a group works everywhere an API does. See
[Route groups](/docs/core-concepts/options#route-groups) for prefixes, nesting,
and inherited options.

## Other handler types

ShiftAPI provides specialized handler methods for different use cases. Each one is available on both `*API` and `*Group`:
