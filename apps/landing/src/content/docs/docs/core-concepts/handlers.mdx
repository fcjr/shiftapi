...
| [`HandleSSE`](/docs/core-concepts/server-sent-events) | Server-Sent Events with a typed writer and auto-generated TypeScript `subscribe` helper |
| [`HandleRaw`](/docs/core-concepts/raw-handlers) | File downloads, WebSocket upgrades, and other responses that need direct `ResponseWriter` access |
