...
```go
shiftapi.Delete(api, "/items/{id}", func(r *http.Request, in DeleteInput) (struct{}, error) {
    deleteItem(in.ID)
    return struct{}{}, nil
...
shiftapi.Delete(api, "/items/{id}", func(r *http.Request, in DeleteInput) (DeleteResponse, error) {
    deleteItem(in.ID)
