...
Pass `EventType[T]` descriptors to `WithEvents` to declare each named event. When `WithEvents` is used, `Send` automatically determines the event name from the concrete Go type — no need to pass the name manually:
