...
shiftapi.HandleWS(api, "GET /rooms/{room}",
    shiftapi.Websocket(
        func(r *http.Request, s *shiftapi.WSSender, in JoinInput) (RoomState, error) {
            ...
            if err != nil {
                return RoomState{}, err // closes connection with StatusInternalError
            }
...
- **After upgrade** — if a `WSOn` handler returns an error, the connection is closed with `WSStatusInternalError` and the error is logged. Use `WithWSOnError` to customize this behavior.
...
`WithWSOnError` registers an error callback that is invoked when a `WSOn` handler returns a non-close error. If not set, the framework logs the error and closes the connection with `WSStatusInternalError`. When set, the callback owns the close decision — the framework will not auto-close after the callback returns.
...
shiftapi.HandleWS(api, "GET /chat",
    shiftapi.Websocket(
        ...
        shiftapi.WSSends{
            ...
            shiftapi.WSMessageType[ErrorMessage]("error"),
        },
    ...
    shiftapi.WithWSOnError(func(r *http.Request, s *shiftapi.WSSender, err error) {
        ...
        s.Send(ErrorMessage{Message: err.Error()})
...
shiftapi.HandleWS(api, "GET /ws", ws,
    ...
    shiftapi.WithWSOnError(func(r *http.Request, s *shiftapi.WSSender, err error) {
        ...
        s.Close(shiftapi.WSStatusInternalError, "error")
    }),
)
```
...
| `WithWSOnError(fn)` | Handle errors from `WSOn` handlers (default: log + close) |
