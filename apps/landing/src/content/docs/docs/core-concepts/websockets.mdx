...
- **`WSSetup[In, State](fn)`** — optional setup function that runs after upgrade, before the dispatch loop. Its return value replaces the parsed input and is passed to all `WSOn` handlers.
- **`WSOnError(fn)`** — optional callback for handler errors. If not set, errors are logged and the connection is closed.
- **`WSOnUnknownMessage(fn)`** — optional callback for unrecognized message types. If not set, unknown types are logged and skipped.
...
```go
func(r *http.Request, ws *shiftapi.WSSender, in In, msg Msg) error
```

- **`r *http.Request`** — the original HTTP request (use `r.Context()` for the connection context)
...
The first type parameter of `WSOn` must match what the dispatch loop passes to handlers:

- **Without `WSSetup`** — handlers receive `In` directly (the parsed input struct from `Websocket[In]`)
- **With `WSSetup[In, State]`** — handlers receive `State` (the value returned by the setup function)

```go
// Without WSSetup — handlers get In directly
shiftapi.Websocket[ChatInput](
    shiftapi.WSOn("message", func(r *http.Request, ws *shiftapi.WSSender, in ChatInput, m UserMessage) error {
        // in is the parsed ChatInput
    }),
    ...
)

// With WSSetup — handlers get State
shiftapi.Websocket[ChatInput](
    ...
    shiftapi.WSOn("message", func(r *http.Request, ws *shiftapi.WSSender, room *Room, m UserMessage) error {
        // room is the State returned by WSSetup
    }),
    ...
)
```
...
```go
ws.Send(ChatMessage{User: "alice", Text: "hi"})
...
Use `WSSetup` to initialize per-connection state after the WebSocket upgrade. The setup function receives the parsed input and returns a state value that replaces `in` in all `WSOn` handlers:
...
shiftapi.HandleWS(api, "GET /chat",
    shiftapi.Websocket[JoinInput](
        shiftapi.WSSetup(func(r *http.Request, ws *shiftapi.WSSender, in JoinInput) (*RoomState, error) {
        ...
        shiftapi.WSOn("message", func(r *http.Request, ws *shiftapi.WSSender, state *RoomState, m UserMessage) error {
            state.Room.Broadcast(ChatMessage{User: state.User.Name, Text: m.Text})
            return nil
        }),
        shiftapi.WSSends(
            shiftapi.MessageType[ChatMessage]("chat"),
...
If the setup function returns an error, the connection is closed with `WSStatusInternalError` and the error is logged. Without `WSSetup`, handlers receive the parsed `In` value directly (the default behavior).
...
- **After upgrade** — if a `WSOn` handler returns an error, the connection is closed with `WSStatusInternalError` and the error is logged. Use `WSOnError` to customize this behavior.
...
`WSOnError` registers an error callback that is invoked when a `WSOn` handler returns a non-close error. If not set, the framework logs the error and closes the connection with `WSStatusInternalError`. When set, the callback owns the close decision — the framework will not auto-close after the callback returns.
...
shiftapi.HandleWS(api, "GET /chat",
    shiftapi.Websocket[struct{}](
        ...
        shiftapi.WSSends(
            shiftapi.MessageType[ChatMessage]("chat"),
            shiftapi.MessageType[ErrorMessage]("error"),
        ),
        shiftapi.WSOnError(func(r *http.Request, ws *shiftapi.WSSender, err error) {
            // Send an error message to the client before closing.
            ws.Send(ErrorMessage{Message: err.Error()})
            ws.Close(shiftapi.WSStatusInternalError, "handler error")
        }),
    ),
)
```
...
`WSOnUnknownMessage` registers a callback for messages whose `type` field does not match any registered `WSOn` handler. If not set, the framework logs the unknown type and continues reading.
...
shiftapi.HandleWS(api, "GET /echo",
    shiftapi.Websocket[struct{}](
        shiftapi.WSOn("echo", func(r *http.Request, ws *shiftapi.WSSender, _ struct{}, msg ClientMsg) error {
            return ws.Send(ServerMsg{Text: msg.Text})
        }),
        shiftapi.WSSends(
            shiftapi.MessageType[ServerMsg]("server"),
        ),
...
shiftapi.HandleWS(api, "GET /chat",
    shiftapi.Websocket[struct{}](
        shiftapi.WSOn("message", func(r *http.Request, ws *shiftapi.WSSender, _ struct{}, m UserMessage) error {
            return ws.Send(ChatMessage{User: "server", Text: m.Text})
        }),
        shiftapi.WSSends(
            shiftapi.MessageType[ChatMessage]("chat"),
...
shiftapi.HandleWS(api, "GET /chat",
    shiftapi.Websocket[struct{}](
        ...
        shiftapi.WSOn("command", func(r *http.Request, ws *shiftapi.WSSender, _ struct{}, cmd UserCommand) error {
            return ws.Send(SystemMessage{Info: "executed: " + cmd.Command})
        }),
        shiftapi.WSSends(
            shiftapi.MessageType[ChatMessage]("chat"),
            shiftapi.MessageType[SystemMessage]("system"),
...
The following callbacks are passed to `Websocket[In]()`, not as route options:

| Callback | Description |
|----------|-------------|
| `WSSetup[In, State](fn)` | Initialize per-connection state after upgrade (default: pass `In` directly) |
| `WSOnError(fn)` | Handle errors from `WSOn` handlers (default: log + close) |
