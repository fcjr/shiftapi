...
Error types registered via `WithError` are included in the generated TypeScript types. The codegen produces a `WSErrorFor<P>` type that narrows `WSError` by close code:
...
The generated `WSChannels` interface includes an `errors` map for each path:
