...
- **Input errors** — if input parsing or validation fails, the framework accepts the WebSocket connection, sends the structured error as the **first message frame**, then closes the connection with an application-defined close code that mirrors the HTTP status (e.g. `4400` for bad request, `4422` for validation errors). This ensures clients can always read the full error details — the browser `WebSocket` API does not expose HTTP response bodies on failed upgrades, so returning a plain HTTP error would be invisible to the client.
- **Setup errors** — if the setup function returns an error that matches a type registered with `WithError[T](status)` (or is a `*ValidationError`), the error is sent as a **typed first frame** and the connection closes with the corresponding `4xxx` code (e.g. `4401` for unauthorized). Unregistered errors close with `WSStatusInternalError` and are logged. This is the recommended way to handle authentication and authorization errors — return them from the setup function so the client receives structured error details.
...
The client receives the full `AuthError` payload as a `WSError`:
...
try {
  ...
  if (err instanceof WSError) {
    console.log(err.code);    // 4401
    console.log(err.details); // { message: "invalid token", realm: "chat" }
  }
}
```

### Handling input errors on the client

The generated TypeScript client detects input errors automatically. When the server closes the connection with a `4xxx` code, the error payload (sent as the first frame) is wrapped in a `WSError`. Input errors are thrown from `for await`, `receive()`, and `send()`. Normal disconnects stop iteration silently.
...
|----------|------|-------------|
| `code` | `number` | The WebSocket close code (e.g. `4400`, `4422`) |
| `message` | `string` | The error message from the server |
| `details` | `unknown` | The full structured error payload |
...
| `WithError[T](code)` | Declare an error type — matched errors from setup are sent as typed first-frame errors with `4xxx` close codes |
